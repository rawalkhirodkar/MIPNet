...
# ---------------------------------------------------------------------------------
def segmix_data(input, target, target_weight, segmentation, alpha=1.0, cutmix_prob=1.0):
...
# ---------------------------------------------------------------------------------
def segmix_data_no_target_weight_update(input, target, target_weight, segmentation, alpha=1.0, cutmix_prob=1.0):
    ''' Returns mixed inputs, pairs of targets, and lambda'''
...
def segmix_criterion(criterion, pred, target, tweighta, tweightb, lam):
    return lam * criterion(pred, target, tweighta) + (1-lam) * criterion(pred, target, tweightb)

def train_segmix(config, train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target, target_weight, meta, segmentation) in enumerate(train_loader):
        ...
        m_input, m_target, m_tweia, m_tweib, lam = segmix_data(input, target,
                target_weight, segmentation, alpha=1.0, cutmix_prob=1.0)

        # m_input, m_target, m_tweia, m_tweib, lam = segmix_data_no_target_weight_update(input, target,
        ...
        if isinstance(outputs, list) and m_tweib is not None:
            loss = segmix_criterion(criterion, outputs[0], m_target, m_tweia, m_tweib, lam)
            for output in outputs[1:]:
                loss += segmix_criterion(criterion, output, m_target, m_tweia, m_tweib, lam)
